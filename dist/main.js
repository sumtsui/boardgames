...
function log() {
  var _console;
  if (window.console) (_console = console).log.apply(_console, arguments);else print.apply(void 0, arguments);
}
var Board = /*#__PURE__*/function () {
  ...
  _createClass(Board, [{
    ...
    value: function setAllObstactles() {
      ...
      var tiles = Array(TILE_TOTAL).fill().map(function (_, i) {
        return i + 1;
      }).filter(function (i) {
        var _EDGES$start$surface, _EDGES$start$surface2;
        var start = _this.cubeMap[i];
        if (!PLAYER_START_TILES.includes(i) && IN_PLAY_SURFACES.includes(start.surface) && !CORNERS.includes(i) && ((_EDGES$start$surface = EDGES[start.surface]) === null || _EDGES$start$surface === void 0 ? void 0 : _EDGES$start$surface.row) !== start.row && ((_EDGES$start$surface2 = EDGES[start.surface]) === null || _EDGES$start$surface2 === void 0 ? void 0 : _EDGES$start$surface2.col) !== start.col) {
          return i;
        }
      });
      ...
      while (obstacleCount < OBSTACLE_TOTAL && attempt < ATTEMPT_TOTAL) {
        ...
        try {
          (function () {
            var startCountOnSurface = obstaclePerSurfaceCount[_this.cubeMap[start].surface];
            if (startCountOnSurface >= Math.ceil(OBSTACLE_TOTAL / 5) || _this.cubeMap[start].surface === 4 && startCountOnSurface === 2) {
              throw "enough obstacle in surface " + _this.cubeMap[start].surface;
            }
            ...
            newObstacle.forEach(function (i) {
              var _this$cubeMap$i;
              if ((_this$cubeMap$i = _this.cubeMap[i]) !== null && _this$cubeMap$i !== void 0 && _this$cubeMap$i.obstacle || PLAYER_START_TILES.includes(i)) {
                throw "OBSTACLE_CREATE_FAIL";
              }
              if (_this.cubeMap[_this.getAdjecentTile(i, "up")].obstacle || _this.cubeMap[_this.getAdjecentTile(i, "down")].obstacle ||
            ...
            obstaclePerSurfaceCount[_this.cubeMap[start].surface] = startCountOnSurface ? startCountOnSurface + 1 : 1;
            obstacleCount++;
            curColorIdx++;
          })();
        } catch (err) {
      ...
      if (a.col !== b.col) {
        if (a.col < b.col) {
          ...
          for (var i = a.col + 1; i <= b.col; i++) {
            var _this$cubeMap$cubeArr = this.cubeMap[cubeArrays[a.surface][a.row][i]],
          ...
          for (var _i = a.col - 1; _i >= b.col; _i--) {
            var _this$cubeMap$cubeArr2 = this.cubeMap[cubeArrays[a.surface][a.row][_i]],
              _obstacle2 = _this$cubeMap$cubeArr2.obstacle,
              _player2 = _this$cubeMap$cubeArr2.player;
            if (_obstacle2) return {
              obstacle: _obstacle2,
              distance: a.col - _i
            };
            if (_player2) return {
        ...
        if (a.row < b.row) {
          ...
          for (var _i2 = a.row + 1; _i2 <= b.row; _i2++) {
            var _this$cubeMap$cubeArr3 = this.cubeMap[cubeArrays[a.surface][_i2][a.col]],
              _obstacle3 = _this$cubeMap$cubeArr3.obstacle,
              _player3 = _this$cubeMap$cubeArr3.player;
            if (_obstacle3) return {
              obstacle: _obstacle3,
              distance: _i2 - a.row
            };
            if (_player3) return {
          ...
          for (var _i3 = a.row - 1; _i3 >= b.row; _i3--) {
            var _this$cubeMap$cubeArr4 = this.cubeMap[cubeArrays[a.surface][_i3][a.col]],
              _obstacle4 = _this$cubeMap$cubeArr4.obstacle,
              _player4 = _this$cubeMap$cubeArr4.player;
            if (_obstacle4) return {
              obstacle: _obstacle4,
              distance: a.row - _i3
            };
            if (_player4) return {
...
var Player = /*#__PURE__*/function () {
  ...
  _createClass(Player, [{
    ...
    value: function move(next) {
      ...
      if (!nextTile || !IN_PLAY_SURFACES.includes(nextTile.surface)) throw "MOVE_FAIL_UNKNOWN_TILE";
      ...
      if (currentTile.surface !== nextTile.surface) {
        if (board.getAdjecentTile(this.current, this._getDirectionChange(next)) !== next) {
          log(board.getAdjecentTile(this.current, this._getDirectionChange(next)));
      ...
      // handle same surface long distance move obstacle check
        ...
        if (thingInPath !== null && thingInPath !== void 0 && thingInPath.obstacle) throw "MOVE_FAIL_OBSTACLE_INBETWEEN";
        if (thingInPath !== null && thingInPath !== void 0 && thingInPath.player) {
    ...
    value: function _getDirectionChange(next) {
      ...
      if ((next - this.current) % 15 === 0) {
        if (next > this.current) return "down";else return "up";
      } else if (Math.abs(next - this.current) <= 4) {
        if (next > this.current) return "right";else return "left";
      } else {
    ...
    value: function _getRawSurrounding(position) {
      ...
      result.up = board.getThingsInPath(current, _objectSpread(_objectSpread({}, current), {}, {
        row: 0
      }));
      result.down = board.getThingsInPath(current, _objectSpread(_objectSpread({}, current), {}, {
        row: 4
      }));
      result.left = board.getThingsInPath(current, _objectSpread(_objectSpread({}, current), {}, {
        col: 0
      }));
      result.right = board.getThingsInPath(current, _objectSpread(_objectSpread({}, current), {}, {
        col: 4
      }));
      for (var key in result) {
        if (result[key] === null) {
          var _board$cubeMap$board$ = board.cubeMap[board.getAdjecentTile(position, key)],
